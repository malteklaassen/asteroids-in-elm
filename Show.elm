...
polygonPlayer = [(0,-10), (10, -15), (0, 15), (-10, -15)]
polygonPlayerAcc : List (Float, Float)
polygonPlayerAcc = polygonPlayer ++ [(-4,-12), (0,-15), (4,-12), (0,-15), (-4,-12)]
...
showPlayer accelerating  p = 
  ...
  List.map 
    (\f -> 
      f <| 
        polygon (List.map identity <| if accelerating then polygonPlayerAcc else polygonPlayer)
    ) <|
  [ filled black -- to hide items BEHIND this one
